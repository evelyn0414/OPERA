...
def extract_vgg_feature(sound_dir_loc, from_signal=False):
    ...
    import urllib
    import sys
    sys.path.append('./baseline/vggish')
    from baseline.vggish import vggish_input
    from baseline.vggish import vggish_params
    from baseline.vggish import vggish_slim

    SR_VGG = 16000  # VGG pretrained model sample rate
    x_data = []

    checkpoint_path = "./baseline/vggish/vggish_model.ckpt"
    pca_params_path = "./baseline/vggish/vggish_pca_params.npz"
...
def extract_clap_feature(sound_dir_loc, single_file=False, version='2022'):
    from baseline.msclap import CLAP
...
def extract_audioMAE_feature(sound_dir_loc, input_sec=10):
    ...
    """
    from tqdm import tqdm
    from baseline.audioMAE.models_mae import mae_vit_small, vit_base_patch16

    encoder_path = "baseline/audioMAE/ViTB_pretrained.pth"
